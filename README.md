```markdown
# 🚀 NetGit - 双平台通用代理服务

[![一键部署到 Netlify](https://www.netlify.com/img/deploy/button.svg)](https://app.netlify.com/start/deploy?repository=https://github.com/2476818641/netgit)
[![一键部署到 Cloudflare Pages](https://static.cloudflareinsights.com/pages/media/deploy-with-cloudflare-pages.svg)](https://deploy.workers.cloudflare.com/?url=https%3A%2F%2Fgithub.com%2F2476818641%2Fnetgit)

**NetGit** 是一个基于 Netlify 和 Cloudflare Pages Functions 的双平台通用代理服务。它旨在提供灵活、快速的代理能力，并最大程度地简化部署流程，让您能轻松拥有自己的专属代理。

## ✨ 主要特性

*   **⚡ 双平台部署:** 同时支持 Netlify 和 Cloudflare，您可以根据自己的偏好和网络环境选择使用。
*   **💨 简化的 Cloudflare 部署:** 无需 `wrangler` CLI，无需手动配置路由。只需在 Cloudflare Pages 中点击几下，即可同时部署静态主页和代理服务。
*   **🔧 配置中心化:** 所有关键配置（如代理白名单、域名等）都集中在 `shared/config.js` 文件中，方便统一管理。
*   **🚀 一键部署:** 提供便捷的部署按钮，让您在几分钟内完成服务的上线。
*   **🎯 特定代理:**
    *   **GitHub & Docker 资源:** 代理 GitHub (raw, release, avatars) 和 Docker (registry, hub) 的资源，解决部分网络环境下访问缓慢或失败的问题。
    *   **Catbox.moe 反向代理:** 安全、匿名地访问 `catbox.moe` 上的内容。

## 📦 项目结构

项目采用模块化的 monorepo 结构，职责清晰：

```text
.
├── netlify/                   # Netlify 平台相关代码
│   ├── build/                 # (构建后生成) 静态主页输出目录
│   ├── functions/             # Netlify Functions (代理逻辑)
│   ├── generate-home.js       # 主页生成脚本
│   ├── netlify.toml           # Netlify 部署配置文件
│   └── package.json           # Netlify 构建依赖
├── pages/                     # Cloudflare Pages 平台相关代码
│   ├── build/                 # (构建后生成) 静态主页和 Worker 输出目录
│   ├── generate-home.js       # 主页生成脚本
│   └── package.json           # Cloudflare Pages 构建依赖
├── shared/                    # 共享配置
│   └── config.js              # 核心配置文件，被所有平台引用
├── worker/                    # (仅供参考) Cloudflare Worker 独立开发目录
│   └── src/
│       └── proxy-worker.js    # 代理核心逻辑 (会被 pages 构建脚本复制)
└── README.md                  # 本说明文件
```

---

## 🛠️ 部署指南

部署此服务非常简单。请按照以下步骤操作。

### 第〇步：准备工作 (必做)

1.  **Fork 本仓库**
    点击此页面右上角的 **"Fork"** 按钮，将此项目复制到您自己的 GitHub 账户下。后续的所有操作都将在您 Fork 后的仓库中进行。

2.  **修改 `shared/config.js`**
    在您 Fork 的仓库中，找到并编辑 `shared/config.js` 文件，将 `GITHUB_USERNAME` 的值修改为您自己的 GitHub 用户名。
    ```javascript
    // shared/config.js
    export const GITHUB_USERNAME = "你的GitHub用户名"; // 替换这里
    ```
    > **提示:** `NETLIFY_HOME_DOMAIN` 和 `CF_PAGES_HOME_DOMAIN` 这两个变量请在完成部署、获得实际域名后再回来修改。

### 第一步：部署到 Cloudflare Pages

这是最简单的部署方式，只需一次部署即可同时拥有主页和代理功能。

1.  **登录到 Cloudflare 仪表板**，进入 **Workers & Pages**。
2.  点击 **"创建应用程序"** -> **"Pages"** -> **"连接到 Git"**。
3.  选择您刚刚 Fork 的 `netgit` 仓库并开始设置。
4.  在 **“构建和部署”** 设置页面，填写以下信息：

    *   **项目名称:** 任意填写 (例如 `my-proxy`)
    *   **生产分支:** `main`
    *   **框架预设:** `None`
    *   **构建设置:**
        *   **根目录:** `/pages`
        *   **构建命令:** `npm install && npm run build`
        *   **构建输出目录:** `build`

5.  点击 **“保存并部署”**。Cloudflare 将会自动构建项目，并将 `pages/build` 目录下的 `index.html` (主页) 和 `_worker.js` (代理服务) 一同部署。
6.  部署成功后，您会得到一个 `.pages.dev` 域名 (例如 `my-proxy.pages.dev`)。**这就是您的 Cloudflare 代理地址！**

### 第二步：部署到 Netlify (可选)

如果您也想在 Netlify 上部署一个副本。

1.  点击仓库顶部的 **"Deploy to Netlify"** 按钮。
2.  登录您的 Netlify 账户并授权 GitHub。
3.  Netlify 会自动识别仓库中的 `netlify.toml` 文件并填充大部分设置。请确认以下信息是否正确：
    *   **Base directory:** `netlify`
    *   **Build command:** `cd netlify && npm install && npm run build`
    *   **Publish directory:** `netlify/build`
4.  点击 **"Deploy site"**。
5.  部署成功后，您会得到一个 `.netlify.app` 的域名。

### 第三步：更新最终域名 (重要)

现在您已经拥有了 Cloudflare 和/或 Netlify 的域名。

1.  回到您 Fork 的 GitHub 仓库中，再次编辑 `shared/config.js` 文件。
2.  将 `CF_PAGES_HOME_DOMAIN` 和 `NETLIFY_HOME_DOMAIN` 的值更新为您刚刚获得的**完整域名**（包含 `https://`）。
    ```javascript
    // shared/config.js
    export const CF_PAGES_HOME_DOMAIN = "https://my-proxy.pages.dev"; // 替换为你的 Cloudflare 域名
    export const NETLIFY_HOME_DOMAIN = "https://my-netlify-site.netlify.app"; // 替换为你的 Netlify 域名
    ```
3.  提交并推送这次修改。Cloudflare 和 Netlify 会自动重新部署，您的主页上的链接现在将是正确的。

---

## ✅ 如何使用

部署完成后，您可以根据主页上的提示使用代理服务。

*   **GitHub 代理**
    ```
    <你的域名>/ghproxy/https://github.com/owner/repo/archive/main.zip
    ```
*   **Docker 代理**
    ```
    <你的域名>/dockerproxy/https://registry-1.docker.io/v2/
    ```
*   **Catbox 代理**
    ```
    <你的域名>/catbox/some_file.jpg
    ```
> 将 `<你的域名>` 替换为您的 Cloudflare 或 Netlify 服务的实际域名。

---

## 💡 常见问题

*   **为什么 Cloudflare 代理不工作？**
    *   请检查您的构建设置是否与指南中的完全一致，特别是 **根目录** 设置为 `/pages`。
    *   `_worker.js` 文件是否成功生成并被复制到了 `pages/build` 目录中。

*   **如何更新代理规则或白名单？**
    *   直接修改 `shared/config.js` 文件。
    *   提交并推送到您的 GitHub `main` 分支。
    *   Cloudflare 和 Netlify 会自动拉取最新代码并重新部署。

## 🤝 贡献

欢迎任何形式的贡献！如果您有改进意见、发现 bug 或希望添加新功能，请随时提交 [Issue](https://github.com/2476818641/netgit/issues) 或 [Pull Request](https://github.com/2476818641/netgit/pulls)。

## 📜 许可证

本项目基于 [MIT](LICENSE) 许可证发布。
```

### **修改点解释**

我将这一行：
`[![一键部署到 Cloudflare Pages](https://deploy.workers.cloudflare.com/button)](https://deploy.workers.cloudflare.com/?url=https%3A%2F%2Fgithub.com%2F2476818641%2Fnetgit)`

修改为了：
`[![一键部署到 Cloudflare Pages](https://static.cloudflareinsights.com/pages/media/deploy-with-cloudflare-pages.svg)](https://deploy.workers.cloudflare.com/?url=https%3A%2F%2Fgithub.com%2F2476818641%2Fnetgit)`

**变化在于：**

*   `[![图片替代文本](图片链接)](跳转链接)` 这是标准的 Markdown 图片链接语法。
*   我把原来的纯文本 `[一键部署到 Cloudflare Pages]` 替换成了 `![一键部署到 Cloudflare Pages](https://static.cloudflareinsights.com/pages/media/deploy-with-cloudflare-pages.svg)`。
*   其中 `https://static.cloudflareinsights.com/pages/media/deploy-with-cloudflare-pages.svg` 就是 Cloudflare 官方提供的 SVG 图片按钮的地址。

这样修改后，GitHub 就会正确地将它渲染成一个可点击的 SVG 图片了，效果和 Netlify 按钮完全一样。
